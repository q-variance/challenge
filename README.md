...
A: The q-variance hypothesis concerns the conditional expectation $\mathbb{E}[v(T)\mid z]$, so for each value of $z$ the relevant empirical object is the mean of $v$ conditional on $z$. Operationally, this corresponds to selecting all periods for which $z$ lies within a small neighbourhood of a given value $z_k$, and computing the sample mean of $v$ within this set. This binning procedure yields a direct estimator of $\mathbb{E}[v(T)\mid z=z_k]$. For example, the intercept $\sigma^2$ is identified as the mean variance among periods with $z \approx 0$, i.e. when the system is in a state of near-zero normalized return. While a regression of $v$ on $z^2$ also targets the conditional expectation, it does so under the maintained assumption that this expectation is exactly quadratic. In the presence of substantial heteroskedasticity and noise in $v$, such a global fit can obscure the underlying relationship. By contrast, binning performs local averaging within conditioning sets, directly estimating the conditional expectation without imposing a functional form, and thereby reduces noise prior to model testing. This makes the structure of $\mathbb{E}[v(T)\mid z]$ both more transparent and more robustly identifiable. An example of how linear regression can give misleading results is provided by the [inverse-gamma notebook](notebooks/invgammavar.ipynb), where the binned curve confirms that the system produces q-variance, as it does by design, but the linear regression is dominated by extreme tail events.
