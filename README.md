...
A: Q-variance is a statement about the conditional expectation function $m(z) := \mathbb{E}[v(T)\mid z]$, in other words the expected variance given a value of $z$. A natural, model-free estimator of $m(z)$ is obtained by binning: for each value $z_k$, we collect all periods for which $z$ lies within a small interval around $z_k$, and compute the sample mean of $v$ within that bin. This yields an estimate $\hat m(z_k)$ of the conditional expectation. For example, the intercept $\sigma^2$ is identified as the mean variance among periods with $z \approx 0$. This binning procedure therefore provides a direct empirical reconstruction of the conditional expectation function $m(z)$, without imposing any parametric form. By contrast, a regression of $v$ on $z^2$ imposes the functional form $m(z) = a + b z^2$ a priori, and estimates its parameters. While useful for testing the specific restriction $b = 1/2$, it does not by itself verify that the conditional expectation is quadratic in $z$. Binning and regression therefore play complementary roles: binning provides a nonparametric estimate of $\mathbb{E}[v(T)\mid z]$, while regression tests whether this function is well approximated by $\sigma^2 + \tfrac12 z^2$. However, given the large intrinsic variability of $v(T)$, direct regression of $v$ on $z^2$ can obscure the structure of the conditional expectation, whereas binning reduces noise by averaging within conditioning sets and makes the functional relationship both visually and statistically transparent. An example of how linear regression can give misleading results is provided by the [inverse-gamma notebook](notebooks/invgammavar.ipynb), where the binned curve confirms that the system produces q-variance, as it does by design, but the linear regression is dominated by extreme tail events.
