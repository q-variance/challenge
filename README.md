...
- a folder `notebooks` with Jupyter notebooks, including `plotqvariance.ipynb` which shows how to compute q-variance for a single asset, `invgammavar.ipynb` to illustrate a simple version of the inverse-gamma model, and `SP500_2008-2021_30m.ipynb` to check q-variance for intraday prices with half-hour time increments
...
- The model should be T-invariant, so changing the period $T$ should not give a very different result, either in terms of the q-variance plot, the estimated quadratic coefficient, or the $z$ distribution.
...
A: No, you can see q-variance over normal time scales such as 20 years of data. It holds not just for stocks, but even for things like Bitcoin or bond yields (see the [article](Q-Variance_Wilmott_July2025.pdf)). Results are not perfect and are subject to noise, so improve after averaging over a number of assets. However if a model of q-variance only works over much longer simulations then it will be sensitive to small changes (e.g. to the exact simulation time) and it also won't be realistic. An example is the [inverse gamma model](notebooks/invgammavar.ipynb).
...
A: In theory the curve should be time-invariant, though in practice there is a small degree of variation with period length, see [Figure 2](Figure_2.png) and [Figure 12](Figure_12_nobin_z0.6.png). The results for individual stocks are of course noisier and also have a different minimum volatility as shown in [Figure 3](Figure_3.png), but taking the average variance over a number of stocks smooths out this noise. The curve is based on a first-order approximation to dynamics, and can hold less well for example when volatility is very low. This is often the case with index data, which represents an average of stock prices. This suppresses the variance and means you are trying to fit a flattened curve. The fit is also less good for very large values of $|z|$. [Figure 1](Figure_1.png) is the fit for $|z| < 0.6$ which covers 93.3 percent of the data. [Figure 9](Figure_9.png) covers |z|<1 and captures 98.31 percent of the data. [Figure 8](Figure_8.png) shows |z|<2, this captures 99.83 percent of the data, and the range in variance covers nearly an order of magnitude. It can be checked that the classical model, where the variance is $\Sigma = \sqrt{2} \sigma$ for all $z$, has $R^2 \leq 0$ with the optimal of zero achieved when the $z$-range is limited to $z \leq \sqrt{3} \Sigma$ as in [Figure 10](Figure_10.png). The data therefore decisively rejects the classical model. You can experiment further using the [Qvar Shiny app](https://david-systemsforecasting.shinyapps.io/qvar/).
